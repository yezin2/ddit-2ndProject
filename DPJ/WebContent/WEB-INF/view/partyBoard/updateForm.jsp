...
<%
	...
	PartyBoardVO pv = (PartyBoardVO) request.getAttribute("partyBoardVO");
...
<div class="container">
	<div class="row">
		<div class="col-md-12 mt-5">
			<h1><i class="fas fa-utensils mr-3"></i>파티글 수정하기</h1>

			<div class="card mb-4">
				<form id="fm" method="post" action="<%=request.getContextPath() %>/PARTY/update.do">
				<div class="card-body">
					<table class="table" >
						<tbody>
							...
							<tr class="d-flex text-left">
								...
								<td class="col-10"><input type="text" class="form-control" name="boardTitle" placeholder="제목을 입력해주세요." value="<%=pv.getBoardTitle() %>" required="required">
								</td>
							</tr>
							<tr class="d-flex">
								...
								<td class="col-lg-2">
									<select class="custom-select" id="restType" name="restCode">
					...
					<textarea class="form-control" name="boardContent" rows="10" placeholder="내용을 입력하세요." required="required"><%=pv.getBoardContent() %></textarea>
				</div>
				<div class="card-footer d-flex justify-content-end">
					<button class="btn btn-outline-success" type="submit"><a>수정 완료</a></button>
					<a class="btn btn-outline-secondary ml-5" href="<%=request.getContextPath() %>/PARTY/main.do">목록으로</a>
				</div>
...
<script>
	$(document).ready(function(){
		...
		// SELECT 기본값 세팅
		$('#price').val('<%=pv.getPrice()%>').prop('selected', true);
		$('#distance').val('<%=pv.getDistance()%>').prop('selected', true);
		$('#restType').val('<%=pv.getRestType()%>').prop('selected', true);
		var end = '<%=pv.getPartyEnd()%>'.replaceAll('/','-');
		$('#partyEnd').val('2021-' + end);
