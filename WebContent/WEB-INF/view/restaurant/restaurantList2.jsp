...
<%
Map<String,String> apiTxtMap = new HashMap(); // 검색 결과에서 중복값 제거
for(int i = 0 ; i < menuListSize ; i++){
	if(apiTxtMap.containsKey(menuList.get(i).getRestName()) == false){
		String latlngs = menuList.get(i).getRestLat() + "," + menuList.get(i).getRestLng();
		apiTxtMap.put(menuList.get(i).getRestName(),latlngs);
	}
}
%>

<% 
ArrayList<String> restArr = new ArrayList<String>(); // 식당목록
ArrayList<String> restLat = new ArrayList<String>(); // 위도목록
ArrayList<String> restLng = new ArrayList<String>(); // 경도목록
%>

var positions = [
	<% 
	Set<Map.Entry<String, String>> mapSet = apiTxtMap.entrySet();
	Iterator<Map.Entry<String, String>> entryIt = mapSet.iterator();
	while(entryIt.hasNext()) { 
		Map.Entry<String, String> entry = entryIt.next();
		String[] latLng = entry.getValue().split(",");
		restArr.add(entry.getKey());
		restLat.add(latLng[0]);
		restLat.add(latLng[1]);
		%>
	    {
	        content: "<div style='width:150px;text-align:center;padding:6px 0;'><%= entry.getKey() %></div>", 
	        latlng: new kakao.maps.LatLng(<%= latLng[0] %>, <%= latLng[1] %>)
	    } <% if(entryIt.hasNext()) { %>
	    		,
	   		<%} %> // close if 문
	   		
	   		restArr.push(<%=entry.getKey()%>);
	   		restLat.push(<%=latLng[0]%>);
	   		restLng.push(<%=latLng[1]%>);
...
for (var i = 0; i < positions.length; i ++) {
    ...
    var resthref = "https://map.kakao.com/link/to/" + restArr[i] + ","+ restLat[i] +"," + restLng[i];
//     console.log ("resthref : " + resthref);
